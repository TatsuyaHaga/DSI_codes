...
Nelem = int(sys.argv[4]) # number of max, min elements in sparse vector
...
#sparsify vectors
def sparsify(v):
    ret = numpy.zeros_like(v)
    idx_sort = numpy.argsort(v)
    idx_max = idx_sort[-Nelem:]
    idx_min = idx_sort[:Nelem]
    ret[idx_max] = v[idx_max]
    ret[idx_min] = v[idx_min]
    return ret
