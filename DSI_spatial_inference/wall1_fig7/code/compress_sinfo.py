...
for ite in range(Niterate):
    ...
    #decorrelation
    """
    demean = Xfrom - nc_sum(Pcol*Xfrom, axis=1, keepdims=True)
    covar = nc_dot(Pcol*demean, nc_transpose(demean))
    var = nc_diag(covar)
    Wcor = covar_offdiag * covar / nc_outer(var, var)
    grad_cor = nc_dot(Wcor, Pcol * demean)
    """
