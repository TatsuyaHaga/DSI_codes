...
G,pos=pickle.load(open(sys.argv[4],"rb"))
...
for trial in range(Nsim):
    ...
    #while shortest_path_length < min_dist or pos[idx_start][0]>=7 or pos[idx_start][1]>=10 or pos[idx_goal][0]<14 or pos[idx_goal][1]<10:
    while shortest_path_length < min_dist or not (pos[idx_start][0]<7 and pos[idx_goal][0]>=14):
...
#path_length_ratio_mean = numpy.mean(path_length_ratio[numpy.array(diff_pathlength)!=0])
...
#pylab.plot([path_length_ratio_mean, path_length_ratio_mean], [0,Nsim], "--", color="black")
