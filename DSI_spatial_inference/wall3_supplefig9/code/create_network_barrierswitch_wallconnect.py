...
for j,k in itertools.combinations(range(N),2):
    ...
    if (xdif<=1 and ydif<=1):
        ...
        if flag_wall(pos[j], pos[k], 10, 10, 0, 1): #wall1-1
            edge_list_A.pop()
            edge_list_AB.pop()
        elif flag_wall(pos[j], pos[k], 10,10,5,0): #wall1-2
            edge_list_A.pop()
            edge_list_AB.pop()
        elif flag_wall(pos[j], pos[k], 15, 15, 5, 1): #wall2-1
            edge_list_B.pop()
            edge_list_AB.pop()
        elif flag_wall(pos[j], pos[k], 5,15,10,0): #door2-2
            edge_list_B.pop()
            edge_list_AB.pop()
